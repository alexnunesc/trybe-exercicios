...
HREF_LINK.addEventListener('click', hrefFunction);
...
INPUT_TEXT.addEventListener('keypress', textFunction);

function textFunction(event) {
  const caractere = event.key; //capturei a tecla
  if (caractere !== 'a') { //verifiquei se é igual, se sim, executar o preventDefault
    event.preventDefault();
  }
}